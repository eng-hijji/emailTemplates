...
export default function Casual({ formInfo, hidden, setVisible }: Props) {
  ...
  return (
    <div className={`relative border rounded-xl p-3 grid gap-3 cursor-pointer hover:bg-slate-100 hover:scale-105 transition duration-300 ease ${hidden.status ? hidden.template === "Casual" ? "block" : "hidden" : ""} `} onClick={set}>
      {hidden.status && hidden.template === "Casual" && (
        <span
          onClick={(e) => {
            e.stopPropagation();
