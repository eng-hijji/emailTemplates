...
export default function Formal({ formInfo, hidden, setVisible }: Props) {
  ...
  return (
    <div className={`border rounded-xl p-3 grid gap-3 cursor-pointer hover:bg-slate-100 hover:scale-105 transition duration-300 ease ${hidden.status ? hidden.template === "Formal" ? "block" : "hidden" : ""} `} onClick={set}>
      ...
      <div className="grid gap-3">
        ...
        <div className="grid gap-1">
          <div>Sincerely,</div>
          <div>{formInfo.firstName} {formInfo.lastName}</div>
        </div>
      </div>
    </div>
  );
